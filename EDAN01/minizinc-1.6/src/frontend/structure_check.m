...
sc_item(Item, !S) :-
    ...
    ;
        ...
        not_in_lang([lang_flatzinc], "annotation items", Locn, !S),
        !S ^ scs_items := [Item | !.S ^ scs_items]
    ;
        (
            RawItem = type_inst_syn_item(_, _, _),
            What = "type-inst synonym items"
        ;
            RawItem = enum_item(_, _, _),
            What = "enum items"
        ),
        not_in_lang([lang_minizinc, lang_flatzinc], What, Locn, !S),
        !S ^ scs_items := [Item | !.S ^ scs_items]
    ;
        RawItem = predfunc_item(Ret, _, _, _, _, MaybeBody),
        (
            Ret = func_ret(_),
            not_in_lang([lang_minizinc, lang_flatzinc], "function items",
                Locn, !S)
        ;
            Ret = test_ret,
            not_in_lang([lang_flatzinc], "test items", Locn, !S),
            (
                MaybeBody = no,
        ...
        ;
            ...
            ;
                MaybeBody = yes(_),
                not_in_lang([lang_flatzinc], "predicate items with bodies",
                    Locn, !S)
            )
        ),
        !S ^ scs_items := [Item | !.S ^ scs_items]
