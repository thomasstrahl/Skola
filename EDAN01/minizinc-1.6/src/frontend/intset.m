...
restrict_min(Min, As0 @ intset(L, U, As1)) = As :-
    ( if Min =< L then
        ...
        As = intset(L, U, restrict_min(Min, As1))
    ).
...
intset_elems_foldl2(P, Ranges, !Acc1, !Acc2) :-
    ...
    ;
        Ranges = intset(L, U, Rest),
        int_fold_up2(P, L + 1, U, !Acc1, !Acc2),
        intset_elems_foldl2(P, Rest, !Acc1, !Acc2)
    ).

    % A version of int.fold_up2 with the second accumulator as an array.
    % XXX this can be removed once the Mercury stdlib supports this mode.
    % (Or more likely we move to non-unique arrays attached to the propagation
    % state.)
    %
:- pred int_fold_up2(pred(int, T, T, U, U), int, int, T, T, U, U).
:- mode int_fold_up2(pred(in, in, out, in, out) is det, in, in,
    in, out, in, out) is det.
:- mode int_fold_up2(pred(in, in, out, array_di, array_uo) is det, in, in,
    in, out, array_di, array_uo) is det.

int_fold_up2(P, Lo, Hi, !A, !B) :-
    ( if    Lo =< Hi
      then  P(Lo, !A, !B), int_fold_up2(P, Lo + 1, Hi, !A, !B)
      else  true
    ).
