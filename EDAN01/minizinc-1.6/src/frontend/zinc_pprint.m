...
doc_raw_item(Lang, type_inst_syn_item(Name, AnnEs, MaybeTIE)) = Doc :-
    (
        AnnEs = [],
        Doc = "type " ++
            mline(doc_name(Name) ++
                eq_mline_doc_maybe_defn(doc_ti_expr(Lang), MaybeTIE)
            )
    ;
        AnnEs = [_ | _],
        Doc = "type " ++
            mline(doc_name(Name) ++ mline(doc_annotations(Lang, AnnEs)) ++
                eq_mline_doc_maybe_defn(doc_ti_expr(Lang), MaybeTIE)
            )
    ).
    ...
    %   <AnnEs> =
    %     <Cases>
doc_raw_item(Lang, enum_item(Name, AnnEs, MaybeDefn)) = Doc :-
    (
        MaybeDefn = enum_defn_no,
        CasesDoc = nil
    ;
        MaybeDefn = enum_defn_flat(FlatCases),
        CasesDoc = " = " ++ mline(doc_enum_flat_cases(FlatCases))
    ;
        MaybeDefn = enum_defn_nonflat(NonFlatCases),
        CasesDoc = " = " ++ mline(doc_enum_nonflat_cases(Lang, NonFlatCases))
    ),
    (
        AnnEs = [],
        Doc = "enum " ++ mline(doc_name(Name) ++ CasesDoc)
    ;
        AnnEs = [_ | _],
        Doc = "enum " ++ mline(doc_name(Name) ++
...
%-----------------------------------------------------------------------------%
    ...
    %   <Case3> }
    %
:- func doc_enum_flat_cases(flat_enum_cases) = doc.

doc_enum_flat_cases(Cases) =
    doc_enclosed_Xs(braces, space, doc_name, Cases).

    % { <Case1>,
    %   <Case2>,
    %   <Case3> }
    %
:- func doc_enum_nonflat_cases(pp_lang, nonflat_enum_cases) = doc.

doc_enum_nonflat_cases(Lang, Cases) =
    doc_enclosed_Xs(braces, space, doc_enum_nonflat_case(Lang), Cases).
    ...
    %   <Field3>)
    %
:- func doc_enum_nonflat_case(pp_lang, nonflat_enum_case) = doc.

doc_enum_nonflat_case(Lang, nonflat_enum_case(CaseName, Fields)) =
    doc_name(CaseName) ++
    ( if Fields = [] then
        nil
      else
        doc_enclosed_Xs_nl(parentheses, nil, doc_ti_expr_and_name(Lang), Fields)
    ).
...
doc_raw_ti_expr(Lang, constrained_raw_ti_expr(TIE, Id, TIConE)) = Doc :-
    Doc =
        parentheses(
            mline(doc_ti_expr(Lang, TIE) ++ ": " ++
                mline(doc_id(Id) ++ space)
            ) ++
            mline("where " ++
                mline(doc_expr(Lang, no, TIConE))
            )
        ).
...
doc_base_ti_expr_tail(Lang, bte_tuple_of(TIEs)) =
    ...
    %   <Field2>,
    %   <Field3>)
doc_base_ti_expr_tail(Lang, bte_record_of(TIEsNames)) =
    doc("record")
        ++ doc_enclosed_Xs_nl(parentheses, nil, doc_ti_expr_and_name(Lang),
                    TIEsNames).
...
doc_base_ti_expr_tail(Lang, bte_range_expr_as_type_expr(E1, E2)) =
    ...
    %     <ArgTIE2>,
    %     <ArgTIE3>))
doc_base_ti_expr_tail(Lang, bte_op(ArgTIEs - RetTIE)) =
    doc("op")
        ++ parentheses(mline(doc_ti_expr(Lang, RetTIE) ++ ": "
            ++ doc_enclosed_Xs_nl(parentheses, nil, doc_ti_expr(Lang),
                        ArgTIEs))).
...
doc_type_inst(Lang, TI) = Doc :-
    % 'ti_overloaded_op' can't be lifted, so we have to handle it here.
    ( if TI = ti_overloaded_op(ProcNsTISigs) then
        TISigs = assoc_list.values(ProcNsTISigs),
        F = ( func(TISig) = doc_type_inst(Lang, ti_op(TISig, no)) ),
        Doc = packed(" or ", map(F, TISigs))
      else
...
doc_raw_expr(Lang,_,lit_simple_array(Es)) =
    ...
    %   default
    %     <E>
doc_raw_expr(Lang,_,lit_indexed_array(IndexEs)) =
    doc_enclosed_Xs(brackets, space, doc_index_expr(Lang), IndexEs).
...
doc_raw_expr(Lang,_,lit_tuple(Es)) =
    ...
    %   <Field2>,
    %   <Field3>)
doc_raw_expr(Lang,_,lit_record(NamedEs)) =
    doc_enclosed_Xs(parentheses, nil, doc_named_expr(Lang), NamedEs).

    % <CaseName>(
    %   <Field1>,
    %   <Field2>,
    %   <Field3>)
doc_raw_expr(Lang,_,lit_nonflat_enum(CaseId, NamedEs)) =
    doc_id(CaseId) ++
        doc_maybe_enclosed_Xs_nl(parentheses, nil, doc_named_expr(Lang),
            NamedEs).
    ...
    %   <E2>,
    %   <E3>)
doc_raw_expr(Lang,_,lit_nonflat_enum_simple(CaseId, Es)) =
    doc_id(CaseId) ++
...
doc_raw_expr(Lang,_,comprehension(simple_array_comp(E), Gens, MaybeWhereE)) =
    ...
    %   default
    %     <E>
doc_raw_expr(Lang,_,comprehension(indexed_array_comp(IndexedE), Gens,
        MaybeWhereE)) =
    doc_comprehension(Lang, brackets, doc_index_expr(Lang), IndexedE, Gens,
            MaybeWhereE).
...
doc_raw_expr(Lang, _, RawExpr) = Doc :-
    ...
    % <TupE>.<Index>
    %
    % Nested tuple access need to be parenthesized:
    %
    % (<TupE>).<Index>
    %
doc_raw_expr(Lang,_,tuple_access(TupE, Index)) = Doc :-
    ( if    TupE ^ raw_expr = tuple_access(_, _)
      then  Parentheses = parentheses
      else  Parentheses = id
    ),
    Doc = Parentheses(doc_expr(Lang, no, TupE)) ++ "." ++ Index.

    % <RecE>.<FieldName>
    %
doc_raw_expr(Lang,_,record_access(RecE, FieldName)) =
    doc_expr(Lang, no, RecE) ++ "." ++ doc_name(FieldName).
...
doc_raw_expr(Lang,_,if_then_else(IfE, ThenE, ElseE)) =
    ...
    %     <CaseOption2>,
    %     <CaseOption3> }
doc_raw_expr(Lang,_,case(E, NamedEs)) =
    "case " ++
        mline(doc_expr(Lang, no, E) ++ space
                ++ doc_enclosed_Xs_nl(braces, space, doc_case_option(Lang),
                                      NamedEs)).
...
doc_name(Name) =
    doc( if is_operator(lang_zinc, Name) then "'" ++ Name ++ "'" else Name ).
