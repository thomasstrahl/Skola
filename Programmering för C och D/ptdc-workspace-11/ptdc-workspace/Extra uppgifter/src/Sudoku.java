public class Sudoku {
	...
	/**
	 * Creates the matrix mat that is used in the class Sudoku.
	 */
	public Sudoku() {
		mat = new int[9][9];
	}
	...
	public boolean solve(int y, int x) {
		...
		for (int a = 0; a < 9; a++) {
			...
			if (!(squareCount(a, b, c) && rowCount(a, b, c) && colCount(a, b, c))) {
				return false;
			}
			}
	...
	private boolean rowCount(int y, int x, int value) {
		int n = 0;
		for (int i = 0; i < 9; i++) {
			if (mat[y][i] == value) {
				n++;
	...
	private boolean colCount(int y, int x, int value) {
		int n = 0;
		for (int i = 0; i < 9; i++) {
			if (mat[i][x] == value) {
				n++;
	...
	private boolean squareCount(int y, int x, int value) {
		int n = 0;
		int tempX = (x / 3) * 3;
		int tempY = (y / 3) * 3;
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (mat[tempY + i][tempX + j] == value) {
					n++;
				}
			}
		}
