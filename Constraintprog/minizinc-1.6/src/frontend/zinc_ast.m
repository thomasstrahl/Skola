...
:- type raw_item
    ...
    ;       enum_item(
                enum_name               :: zinc_name,
                enum_annotations        :: exprs,
                enum_maybe_defn         :: maybe_enum_defn
            )
    ;       type_inst_syn_item(
                type_inst_syn_name          :: zinc_name,
                type_inst_syn_annotations   :: exprs,
                type_inst_syn_maybe_ti_expr :: maybe(ti_expr)
            )
    ;       annotation_item(
...
:- type maybe_enum_defn
    --->    enum_defn_no
    ;       enum_defn_flat(
                flat_cases :: flat_enum_cases
            )
    ;       enum_defn_nonflat(
                nonflat_cases :: nonflat_enum_cases
            ).

:- type flat_enum_case == zinc_name.
:- type flat_enum_cases == list(flat_enum_case).

:- type nonflat_enum_cases == list(nonflat_enum_case).
:- type nonflat_enum_case
    --->    nonflat_enum_case(
                zinc_name,                   % case name
                ti_exprs_and_names      % fields (possibly empty)
            ).
...
:- type base_ti_expr_tail
    ...
    ;       bte_record_of(ti_exprs_and_names)
...
:- type ti_expr_sig  == pair(ti_exprs, ti_expr).
    ...
    % The name is the case name, the ti_exprs_and_names are for the fields.
:- type enum_ti_expr_sig == list(pair(zinc_name, ti_exprs_and_names)).
...
:- type raw_expr
    % Atomic expressions
            ...
            % flat enum case name, or enum/set type name.
    --->    ident(id)
    ;       lit(literal)
            % New 'app' exprs can be constructed with 'raw_app_init'.
            % Nb: see comment above 'lit_nonflat_enum_simple'.
    ...
    ;       lit_indexed_array(index_exprs)
    ;       lit_tuple(exprs)
    ;       lit_record(named_exprs)
    ;       lit_nonflat_enum(id, named_exprs)
            % Nb: 'lit_nonflat_simple' and 'app' expressions
            % syntactically indistinguishable, so the parser always produces
            % 'app' expressions.  If an 'app' should really be a
            % 'lit_nonflat_enum_simple', it gets converted during symbol
            % checking.
    ;       lit_nonflat_enum_simple(id, exprs)
    ;       lit_ann(id, exprs)
    ;       comprehension(
                comprehension_kind,
                generators,
    ...
    ;       tuple_access(expr, int)
            % The field-name is stored as a 'name' rather than an 'id' because
            % field-names inhabit a different namespace to all other names.
    ;       record_access(expr, zinc_name)
    ;       anon_var                    % `_'
...
lift_type_inst_to_ti_expr(Locn, TI) = TIE :-
    (   TI = ti_par_bottom,  VarPar = par, BaseTIETail = bte_bottom
    ;   TI = ti_var_bottom,  VarPar = var,  BaseTIETail = bte_bottom
    ;   TI = ti_par_bool,    VarPar = par, BaseTIETail = bte_bool
    ;   TI = ti_var_bool,    VarPar = var,  BaseTIETail = bte_bool
    ;   TI = ti_par_int,     VarPar = par, BaseTIETail = bte_int
    ;   TI = ti_var_int,     VarPar = var,  BaseTIETail = bte_int
    ;   TI = ti_par_float,   VarPar = par, BaseTIETail = bte_float
    ;   TI = ti_var_float,   VarPar = var,  BaseTIETail = bte_float
    ;   TI = ti_par_string,  VarPar = par, BaseTIETail = bte_string
    ;   TI = ti_ann,         VarPar = par, BaseTIETail = bte_ann
    ;
        TI = ti_par_enum(X),
        VarPar = par,
        BaseTIETail = bte_ident(id_global(X))
    ;
        TI = ti_var_enum(X),
        VarPar = var,
        BaseTIETail = bte_ident(id_global(X))
    ...
    ;
        ...
        VarPar = par,
        TIAndId_2_TIEAndId = (
            func(PTI - Id) = lift_type_inst_to_ti_expr(Locn, PTI) - Id
        ),
        BaseTIETail = bte_record_of(map(TIAndId_2_TIEAndId, TIsNames))
    ;
        TI = ti_op(ArgTIs - RetTI,_),
        VarPar = par,
        ArgTIEs = map(lift_type_inst_to_ti_expr(Locn), ArgTIs),
        RetTIE = lift_type_inst_to_ti_expr(Locn, RetTI),
