...
:- type symbol
    ...
    ;       sym_type_inst_synonym(type_syn_info)
    ;       sym_enum(enum_info)
    ;       sym_enum_case_name(
                % Name of the parent enum type.
                enum_case_name_parent :: zinc_name
            )
    .
...
:- type type_syn_info
    --->    type_syn_info_undefined
    ;       type_syn_info_defined(
                tsid_type_inst    :: type_inst,         % Type inst.
                tsid_use_as_value :: is_usable_as_value,% is_usable_as_value
                tsid_defn_reqd    :: is_defn_required, 
                % Must be yes for type-insts that must be defined if used.
                % See sym_variable above.
                tsid_is_finite    :: is_finite
            ).
...
:- type enum_info
    --->    enum_info_undefined
    ;       enum_info_flat(flat_enum_cases)
    ;       enum_info_nonflat(
                is_finite,          % Is finite.
                nfe_case_infos      % Case info.
            ).

:- type nfe_case_infos == list(nfe_case_info).
:- type nfe_case_info
    --->    nfe_case_info(
                nfe_cinfo_name   :: zinc_name,
                nfe_cinfo_fields :: nfe_field_infos
            ).

:- type nfe_field_infos == list(nfe_field_info).
:- type nfe_field_info
    --->    nfe_field_info(
                nfe_finfo_name    :: zinc_name,
                nfe_finfo_ti_expr :: ti_expr,
                nfe_finfo_ti_sig  :: type_inst
            ).

    % Return the fields for a given case.
    % Fails if the case is not present in the given case infos.
    %
:- pred get_field_infos_for_case(nfe_case_infos::in, zinc_name::in,
    nfe_field_infos::out) is semidet.

    % Return the list of case names for the given non-flat enum.
    % The resulting list of case names is in declaration order.
    %
:- func get_nfe_case_names(nfe_case_infos) = list(zinc_name).
...
:- instance show(symbol) where [
    sym_variable(_,VarKind,_,_,_)^show = VarKind^show,
    sym_operation(OpKind,_)      ^show = OpKind^show,
    sym_annotation(_)            ^show = "annotation",
    sym_type_inst_synonym(_)     ^show = "type-inst synonym",
    sym_enum(_)                  ^show = "enum",
    sym_enum_case_name(_)        ^show = "enum case name"
].


:- instance show(variable_kind) where [
    global_var         ^show = "variable",
    type_constraint_var^show = "type constraint variable",
...
ti_is_fixed_2(SymTbl, ti_record(TIsNs)) =
    bool.and_list(map(ti_is_fixed_2(SymTbl), assoc_list.keys(TIsNs))).
ti_is_fixed_2(SymTbl,ti_par_enum(Name)) = Out :-
    Sym = find_existing_global_symbol(Name, SymTbl),
    (
        Sym = sym_enum(EnumInfo),
        (
            EnumInfo = enum_info_nonflat(_, CaseInfos),
            Out = (if nfe_cases_are_fixed(SymTbl, CaseInfos) then yes else no)
        ;
            % If we're using --model-check-only, we might have an undefined
            % enum.  Such an enum must be flat, and thus fixed.
    ...
    ;
        ...
        unexpected($pred ++ ": something wrong with enum symbol " ++ Name)
    ).
ti_is_fixed_2(_,ti_var_enum(_))     = no.
...
:- pred nfe_cases_are_fixed(symbol_table::in, nfe_case_infos::in) is semidet.

nfe_cases_are_fixed(SymTbl, CaseInfos) :-
     list.all_true(nfe_case_is_fixed(SymTbl), CaseInfos).

:- pred nfe_case_is_fixed(symbol_table::in, nfe_case_info::in) is semidet.

nfe_case_is_fixed(SymTbl, CaseInfo) :-
    CaseInfo = nfe_case_info(_, FieldInfos),
    list.all_true(nfe_field_is_fixed(SymTbl), FieldInfos).

:- pred nfe_field_is_fixed(symbol_table::in, nfe_field_info::in) is semidet.

nfe_field_is_fixed(SymTbl, FieldInfo) :-
    FieldInfo = nfe_field_info(_, _, TI),
    ti_is_fixed_2(SymTbl, TI) = yes.
...
parify_type_inst(SymTbl, ti_record(TIsNames)) = RecTI :-
    F = ( func(P_TI - P_Name) = parify_type_inst(SymTbl, P_TI) - P_Name ),
...
parify_type_inst(_,ti_op(_,_)         ) = ti_top.
parify_type_inst(_,ti_error           ) = ti_error.
parify_type_inst(SymTbl, ti_par_enum(EnumName)) = TI :-
    % An enum type can only be parified if every field within it is already
    % parified. 
    Sym = find_existing_global_symbol(EnumName, SymTbl),
    (
        Sym = sym_enum(EnumInfo),
        (
            EnumInfo = enum_info_nonflat(_, CaseInfos),
            ( if    list.all_true(is_case_parified(SymTbl), CaseInfos)
              then  TI = ti_par_enum(EnumName)
              else  TI = ti_top
            )
        ;
            % If we're using --model-check-only, we might have an undefined
            % enum.   Such an enum must be flat, and thus already par.
            ( EnumInfo = enum_info_flat(_)
            ; EnumInfo = enum_info_undefined
            ),
    ...
    ; 
        ...
        unexpected($pred ++ ": problem with enum symbol")
    ).
    % This must be a flat enum to be par in the first place.  So parifying is
    % trivial.
parify_type_inst(_,ti_var_enum(Name)  ) = ti_par_enum(Name).
parify_type_inst(_,TI) = _ :-
    ( TI = ti_overloaded_op(_)
    ; TI = ti_unknown
    ),
    unexpected($pred ++ ": " ++ TI^string).

:- pred is_case_parified(symbol_table::in, nfe_case_info::in) is semidet.

is_case_parified(SymTbl, CaseInfo) :-
    CaseInfo = nfe_case_info(_, FieldInfos),
    list.all_true(is_field_parified(SymTbl), FieldInfos). 

:- pred is_field_parified(symbol_table::in, nfe_field_info::in) is semidet.

is_field_parified(SymTbl, FieldInfo) :-
    FieldInfo = nfe_field_info(_, _, TI),
    TI = parify_type_inst(SymTbl, TI).

%-----------------------------------------------------------------------------%

get_field_infos_for_case(CaseInfos0, Name, ResultFields) :-
    (
        CaseInfos0 = [],
        false
    ;
        CaseInfos0 = [CaseInfo | CaseInfos],
        CaseInfo = nfe_case_info(CaseName, CaseFields),
        ( if    CaseName = Name
          then  ResultFields = CaseFields
          else  get_field_infos_for_case(CaseInfos, Name, ResultFields)
        )
    ).

get_nfe_case_names([]) = [].
get_nfe_case_names([CaseInfo | CaseInfos]) = [Name | Names] :-
    CaseInfo = nfe_case_info(Name, _),
    Names = get_nfe_case_names(CaseInfos).
