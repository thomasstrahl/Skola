...
top_sort_ast(Lang, Items, ATSortedItems - !:GraphAssocList, !:Errs, Warns) :-
    else_unexpected(is_lang(Lang, [lang_zinc, lang_minizinc]),
    ...
    some [!Graph] (
        ...
        IsNotAssignmentWithFakeCycle = (
            pred((DefItem - UseItem)::in) is semidet :-
                ( if DefItem = UseItem,
                     DefItem = item(assign_item(Name, E), _),
                     not set.member(Name, uses_in_expr(E)) then
        ...
        CheckIfItemIsCyclic = (
            pred((DefItem - UseItem)::in, !.CycleErrs::in, !:CycleErrs::out)
                is det :-
                ( if DefItem = UseItem then
                    cyclic_defn_error(DefItem, !CycleErrs)
                  else
                    true
                )
        ),
...
do_defs(Item @ item(RawItem, _Locn), !DefMMap, !Graph) :-
    ...
    AddDefdNameAndItem = (
        pred(Name::in, !.DefMMap::in, !:DefMMap::out) is det :-
...
cyclic_defn_error(item(RawItem, Locn), !Errs) :-
    Defs = defs_in_raw_item(RawItem),
     ( if is_singleton(Defs, Def) then
        CyclicName = Def
      else if RawItem = enum_item(Name, _, _) then
        % enum items can def more than one thing (the enum name, plus the
        % case names).  If that happens, get the enum name and use that in
        % the error.
        CyclicName = Name
      else
        unexpected($pred, "something went wrong")
    ),
    ErrMsg = [words("cycle error: cycle in definition of `" ++ CyclicName ++
                "'")],
...
defs_in_raw_item(RawItem) = Defs :-
    ...
    ; RawItem = type_inst_syn_item(Name,_,_)
...
defs_in_raw_item(enum_item(Name, _, MaybeDefn)) = Defs :-
    (
        MaybeDefn = enum_defn_no,
        Defs = make_singleton_set(Name)
    ; 
        (
            MaybeDefn = enum_defn_flat(CaseNames)
        ;
            MaybeDefn = enum_defn_nonflat(Cases),
            GetEnumCaseName = (func(nonflat_enum_case(CaseName, _)) = CaseName),
            CaseNames = list.map(GetEnumCaseName, Cases)
        ),
        CasesDefs = list_to_set(CaseNames),
        Defs = make_singleton_set(Name) `union` CasesDefs
    ).
...
uses_in_raw_item(enum_item(_EnumName, AnnEs, MaybeDefn)) = Uses :-
    UsesInAnnEs = uses_in_Xs(uses_in_expr, AnnEs),
    (
        ( MaybeDefn = enum_defn_no
        ; MaybeDefn = enum_defn_flat(_)
        ),
        Uses = UsesInAnnEs
    ;
        MaybeDefn = enum_defn_nonflat(Cases),
        Uses = uses_in_Xs(uses_in_nonflat_enum_case, Cases) `union`
                          UsesInAnnEs
    ).

uses_in_raw_item(solve_item(SolveKind, AnnEs)) = Uses :-
    (   SolveKind = satisfy,     UsesInSolveKind = set.init
    ;   SolveKind = minimize(E), UsesInSolveKind = uses_in_expr(E)
    ;   SolveKind = maximize(E), UsesInSolveKind = uses_in_expr(E)
    ),
...
uses_in_raw_item(type_inst_syn_item(_Name, AnnEs, MaybeTIE)) = Uses :-
    Uses = uses_in_maybe_X(uses_in_ti_expr, MaybeTIE) `union`
           uses_in_Xs(uses_in_expr, AnnEs).
...
uses_in_raw_item(RawItem) = Uses :-
    ...
    Uses = RetUses
        `union` uses_in_Xs(uses_in_ti_expr, ParamTIs)
        `union` uses_in_Xs(uses_in_expr, AnnEs)
        `union` ( uses_in_maybe_X(uses_in_expr, MaybeBodyE)
                    `difference` uses_in_Xs(uses_in_id, ParamIds)).

%-----------------------------------------------------------------------------%

:- func uses_in_nonflat_enum_case(nonflat_enum_case) = set(zinc_name).

uses_in_nonflat_enum_case(nonflat_enum_case(_,Fields)) = Uses :-
    FieldsTIs = assoc_list.keys(Fields),
    Uses = uses_in_Xs(uses_in_ti_expr, FieldsTIs).
...
uses_in_raw_expr(RawE) = Uses :-
    (   RawE = ident(Id), Uses = uses_in_id(Id)
    ;   RawE = anon_var,  Uses = set.init
    ;   RawE = lit(_Lit), Uses = set.init
    ;
        ( RawE = lit_set(Es)
    ...
    ;
        ...
        assoc_list.keys_and_values(IndexedEs, IndexEs, ValueEs),
        Uses = uses_in_Xs(uses_in_expr, IndexEs)
                `union` uses_in_Xs(uses_in_expr, ValueEs)
    ;
        RawE = lit_record(NamedEs),
        Uses = uses_in_Xs(uses_in_expr, assoc_list.values(NamedEs))
    ; 
        RawE = lit_nonflat_enum(EnumCaseId, NamedEs),
        Uses = uses_in_id(EnumCaseId)
                `union` uses_in_Xs(uses_in_expr, assoc_list.values(NamedEs))
    ; 
        ...
        VarsAndUsesInGenerator = (
            ...
            NAllUses =
                AllUses `union` (uses_in_expr(GenE) `difference` AllVars)
        ),
        ...
        WhereUses = 
            uses_in_maybe_X(uses_in_expr, MaybeWhereE) `difference` GenVars,
        (   ( Kind = set_comp(E)
            ; Kind = simple_array_comp(E)
            ),
            HeadUses = uses_in_expr(E) `difference` GenVars
        ;   
            Kind = indexed_array_comp(IndexE - ValueE),
            HeadUses = (uses_in_expr(IndexE) `union` uses_in_expr(ValueE)
                        `difference` GenVars)
        ),
    ...
    ;
        RawE = tuple_access(TupE, _FieldN),
        Uses = uses_in_expr(TupE)
    ;   
        RawE = record_access(RecE, _FieldName),
        Uses = uses_in_expr(RecE)
    ;
        RawE = if_then_else(IfE, ThenE, ElseE),
        Uses = uses_in_Xs(uses_in_expr, [IfE, ThenE, ElseE])
    ;
        RawE = case(CaseE, LabelIdsEs),
        GetLabelName = (func(Id - _) = Id ^ id_name),
        LabelNames = list.map(GetLabelName, LabelIdsEs),
        % We include the label names in the uses.  This may seem strange, but
        % it's so that if the user incorrectly uses something other than a
        % case name as a label (eg. a variable name), then that symbol will
        % have already been installed in the symbol table, and we'll get a "X
        % used as a case label" error.  Without this, we can get a "X is
        % undeclared" error, which is misleading.
        Uses = uses_in_expr(CaseE)
                `union` list_to_set(LabelNames)
                `union` uses_in_Xs(uses_in_expr, assoc_list.values(LabelIdsEs))
    ;
        RawE = app(Id, _ProcN, _AppKind, ArgEs),
        Uses = make_singleton_set(Id ^ id_name)
                `union` uses_in_Xs(uses_in_expr, ArgEs)
    ;
    ...
    ;
        ...
        Uses = uses_in_Xs(uses_in_expr, Es)
    ;
        % Shouldn't see these just after parsing.
...
uses_in_raw_ti_expr(raw_ti_expr(_VarPar, BaseTIETail)) = Uses :-
    ...
    ;
        ...
        Uses = uses_in_Xs(uses_in_ti_expr, ArgTIEs) `union`
                uses_in_ti_expr(RetTIE)
    ).

uses_in_raw_ti_expr(constrained_raw_ti_expr(TIE, VarId, TIConE)) =
    % Nb: We exclude the type-inst constraint variable from the TICon uses,
    % because it's internal.
    uses_in_ti_expr(TIE) `union`
        (uses_in_expr(TIConE) `difference`
         make_singleton_set(VarId ^ id_name)).
...
pprint_top_sort_graph(TopSortGraph, IgnoredFiles, !IO) :-
    ...
    WritePair = (
        pred((Item1 - Item2)::in, !.IO::di, !:IO::uo) is det :-
            io.write_string(Item1^item_src_locn^show, !IO),
            io.write_string(" < ",                    !IO),
            io.write_string(Item2^item_src_locn^show, !IO)
    ),
    ItemsNotInIgnoredFiles = (
        pred((Item1 - Item2)::in) is semidet :-
...
categorise_item(Item, !Preds, !Vars, !Constraints, !Solves, !Outputs) :-
    ...
    ( if RawItem = predfunc_item(_, _, _, _, _, _) then
      ...
      else if RawItem = var_decl_item(_, _, _, _) then
        list.cons(Item, !Vars)
      else if RawItem = constraint_item(_) then
        list.cons(Item, !Constraints)
      else if RawItem = solve_item(_, _) then
        list.cons(Item, !Solves)
      else if RawItem = output_item(_) then
