...
process_var_decl_for_ozn(Env, !.TIExpr, Name, Assignment, !Pars, !Vars,
    ...
    ;
        ...
        unexpected($pred, "constrained type-inst in MiniZinc")
    ),
    ( if bte_contains_ann(BaseTIExprTail) then
        ...
        then
            ...
            ( if VarPar0 = ElemTIExpr ^ raw_ti_expr ^ var_par
            then VarPar = VarPar0
            else unexpected($pred, "constrained type-inst in MiniZinc")
            )
        else
...
remove_constrained_float_ti_expr(!.TIExpr) = !:TIExpr :-
    ...
    (
        ...
        !:TIExpr = ti_expr(RawTIExpr, ExprInfo)
    ;
        RawTIExpr0 = constrained_raw_ti_expr(_, _, _),
        unexpected($pred, "constrained type-inst in MiniZinc")
    ).
...
subst_scalar_params_in_comp_kind(Map, !.Kind) = !:Kind :-
    ...
    ;
        !.Kind = indexed_array_comp(_),
        unexpected($pred, "indexed array comprehension in MiniZinc")
    ).
...
subst_scalar_params_in_ti_expr(Map, !.TIExpr) = !:TIExpr :-
    ...
    ;
        RawTIExpr0 = constrained_raw_ti_expr(_, _, _),
        unexpected($pred, "constrainted type-inst in MiniZinc")
    ).
