...
:- instance global(gContent, contentStore).
...
getEntity(internal(Entity)) -->
    return(Entity).
getEntity(external(system(_SystemId))) -->
    { Entity = "" },
    return(Entity).
% XXX - For XML-FlatZinc we don't have external entities, so
% this has been disabled so that xml2fzn doesn't require flatzinc.dtd.
% Likewise, for the PUBLICID case below.
    %get(gDirs, dirs(Dirs)),
...
%	error(Msg)
%    ).
getEntity(external(public(_PublicId, _SystemId))) -->
    {Entity = ""},
    return(Entity).
%    ( { SystemId \= "" } ->
